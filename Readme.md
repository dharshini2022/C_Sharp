**C# Tasks**
**Task1:** Console - based Factorial Program.
* Learnt about basic data types, control structures and methods in C#.
* Implemented input validation using TryParse.
* Used recursion to implement the factorial logic.

**Task2:** Simple Object - Oriented Programming Exercise
* Implemented class and object relation
* Implemented constructor based initiation.
* Learnt about pillars of oops

**Task3:** Basic Collection and String Manipulation tasks
* Implemented a dynamic List of String data type
* Implemented option based add, remove and print functionalities
* Implemented string functions like Trim() and ToUpper()

**Task4:** Collections and Linq
* Implemented a dynamic list
* Used LINQ functions filter (where) and ascending order (orderBy)

**Task5:** File Handling
* Implemented create, read, write and append functions to file and folders.
* Used Exisits, Streamwriter and ReadAllText functions.
* Implemented exception handling using try catch.

**Task6:** Delegates and Event Handling
* Implemented a Delegate based Counter Event Handling that works with respect to a threshold value.
* Threshold reached --> Event Triggered --> Handlers wrt Delegates get executed
* A delegate in C# is a type-safe function pointer that holds references to methods with a specific signature and allows them to be invoked dynamically.